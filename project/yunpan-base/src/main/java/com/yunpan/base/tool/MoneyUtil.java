...
public abstract class MoneyUtil {
	...
	public static Long parseAmountToRoundCent(Long amount) {
		return (new BigDecimal(amount).divide(new BigDecimal("100"))).setScale(
	 ...
	 * 解析金额（从分单位到带有两位小数的元单位）
	 */
	public static Double parseFromFenAmountToRMB(Double amount) {
		try {
			BigDecimal bigDecimal = new BigDecimal(amount);
			return bigDecimal.divide(new BigDecimal("100"), 2,
	...
	public static String parseFromUSToReadableAmount(String amount)
			throws ParseException {
		NumberFormat format = java.text.NumberFormat
				.getNumberInstance(java.util.Locale.US);
		return parseFromRMBToReadableAmount(String
				.valueOf(format.parse(amount)));
	}
